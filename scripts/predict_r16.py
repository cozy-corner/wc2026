...
susp_by_match = {}
for s in yaml.safe_load(open("suspensions.yaml"))["suspensions"]:
    if s["role"] == "starter":
        susp_by_match.setdefault(s["misses"], {})
        susp_by_match[s["misses"]][s["team"]] = susp_by_match[s["misses"]].get(s["team"], 0) + SUSP
...
for m in B["round_of_16"]:
    ...
    sa = susp_by_match.get(m["id"], {}).get(a, 0)
    sb = susp_by_match.get(m["id"], {}).get(b, 0)
    qa, qb = Q(a) + ha - sa, Q(b) + hb - sb
    pa = logistic((qa - qb) / T)
